...
if (isLoggedIn()) {
    ...
    $userRole = in_array($_SESSION['user_role'] ?? '', $allowedRoles) ? $_SESSION['user_role'] : '';
}
?>
<nav style="background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(10px); box-shadow: 0 4px 15px rgba(233,30,99,0.05); position: sticky; top: 0; z-index: 1000; border-bottom: 1px solid rgba(255,255,255,0.5);">
    <div class="container" style="display: flex; align-items: center; justify-content: space-between; height: 80px;">
        ...
        <!-- Logo -->
        <a href="<?= SITE_URL ?>/index.php" style="display: flex; align-items: center; gap: 10px; text-decoration: none;">
            <div style="width: 52px; height: 52px; border-radius: 50%; background: white; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 16px rgba(233,30,99,0.25); overflow: hidden; flex-shrink: 0; border: 2px solid rgba(233,30,99,0.15);">
        ...
        <!-- Desktop Menu -->
        <div style="display: flex; align-items: center; gap: var(--space-lg);" class="desktop-menu">
            ...
            <a href="<?= SITE_URL ?>/services.php" style="color: <?= $currentPage == 'services.php' ? 'var(--main-pink)' : 'var(--dark-gray)' ?>; font-weight: <?= $currentPage == 'services.php' ? '600' : '500' ?>; font-size: 15px;">Services</a>
            ...
            <a href="<?= SITE_URL ?>/contact.php" style="color: <?= $currentPage == 'contact.php' ? 'var(--main-pink)' : 'var(--dark-gray)' ?>; font-weight: <?= $currentPage == 'contact.php' ? '600' : '500' ?>; font-size: 15px;">Contact</a>
            
            <div style="width: 1px; height: 24px; background: #E0E0E0; margin: 0 var(--space-xs);"></div>
            
            <?php if(isLoggedIn() && $userRole): ?>
                <a href="<?= SITE_URL ?>/<?= $userRole ?>/dashboard.php" style="color: var(--dark-gray); font-weight: 600;">Dashboard</a>
...
<div id="mobileMenu" style="position: fixed; top: 80px; left: 0; width: 100%; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); box-shadow: var(--shadow-md); z-index: 999; display: none; flex-direction: column; padding: var(--space-md);">
    ...
    <a href="<?= SITE_URL ?>/services.php" style="padding: 12px; border-bottom: 1px solid rgba(0,0,0,0.05); color: var(--dark-gray); font-weight: 500;">Services</a>
    ...
    <a href="<?= SITE_URL ?>/contact.php" style="padding: 12px; border-bottom: 1px solid rgba(0,0,0,0.05); color: var(--dark-gray); font-weight: 500;">Contact</a>
    
    <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 16px;">
        <?php if(isLoggedIn() && $userRole): ?>
            <a href="<?= SITE_URL ?>/<?= $userRole ?>/dashboard.php" class="btn btn-outline" style="width: 100%;">Dashboard</a>
